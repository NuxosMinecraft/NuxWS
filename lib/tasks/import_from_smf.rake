...
namespace :smf do
  task :import => :environment do
    ...
    topics.each do |t|
      ...
      t_nuxws = Topic.find_or_create_by_title(:title => t["title"], :username => t["username"], :content => t["content"], :created_at => t["created_at"], :last_message_at => t["last_message_at"], :forum_id => forum.id)
      ...
      messages.each do |m|
        m_nuxws = Message.find_or_create_by_content(:username => m["username"], :title => m["title"], :content => m["content"], :created_at => m["created_at"], :updated_at => m["updated_at"], :last_edit_by => m["last_edit_by"], :topic_id => t_nuxws.id)
